// 33A - What is for dinner? (https://codeforces.com/contest/33/problem/A)
...
function main() {
    // n (quantidade de dentes), m (linhas de dentes), k (quantidade de crustáceos)
    const inputs = readline().split(' ').map((element) => Number(element));
    
    let rows = Array(inputs[1]);
...
'1 16'];
    ...
    for (let i = 0; i < inputs[0]; i++) {
        let row_data = readline().split(' ').map((element) => Number(element));
        
        if (rows[row_data[0] - 1] === undefined) {
            rows[row_data[0] - 1] = row_data[1];  
        } else if (row_data[1] < rows[row_data[0] - 1]) {
            rows[row_data[0] - 1] = row_data[1];  
        }
    }
    ...
    for (let i = 0; i < rows.length; i++) {
        sum_teeth += Number(rows[i]);
    };

    if (sum_teeth > inputs[2]) { return console.log(inputs[2]) };
    return console.log(sum_teeth)
}